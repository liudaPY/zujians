...
#import "tztAutoScrollLabel.h"
#import "tztHKStatusManger.h"

#define kDefaultScrollDuration 10
#define kDefaultSrollVelocity 27.5
#define kDefaultPauseTime 0.0
#define kDefaultPadding 50.0
#define kDefaultDelay 0.0

@interface tztAutoScrollLabel()
...
@end
@implementation tztAutoScrollLabel
...
- (void)setupZScrollLabel {
    ...
    _manualStop = NO;
    _labelAlignment = zzScrollLabelAlignmentCenter;
...
- (void)setLabelAlignment:(tztScrollLabelAlignment)labelAlignment {
...
- (void)setLabelsFrame {
    ...
    if (self.label.frame.size.width > self.frame.size.width) {
        ...
        if (self.labelAlignment == zzScrollLabelAlignmentLeft) {
            center.x = self.label.frame.size.width / 2.0;
        } else if (self.labelAlignment == zzScrollLabelAlignmentCenter) {
...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context {
    self.scrolling = NO;
    self.text = [tztHKStatusManger getShareInstance].scrollText;
    if (self.label.frame.size.width > self.frame.size.width){
